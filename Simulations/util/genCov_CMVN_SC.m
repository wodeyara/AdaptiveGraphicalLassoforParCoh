...
randomphase = (pi/2+randn(length(useAreas))*.25) * double(~eye(length(useAreas))) .*(SC(useAreas, useAreas)>0) ;
% first we will normalize each row sum to 1.
cnt  = 1;
err = 1;
while err ~=0 && cnt < 1e3 % ensure that matrix is positive definite
    r = abs(100+randn(720,1)*30); % random values for edges
    G = double(triu(SC(useAreas, useAreas)>0,1));
    G(G>0) = r;
    G = G + G';
    orig_G = G;
    origNetwork = abs(orig_G)>0;
    G = triu(G .*exp(1i * randomphase),1); % complex values established for precision
    G = G+G';
    ...
    [~,err] = chol(Q); % test whether matrix is actually positive definite
    cnt  = cnt +1;
end
...
clear data compData orig_CohMat orig_CovMat r frequency axonalSpeed randomphase cnt err covMatCSD
